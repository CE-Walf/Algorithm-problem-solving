...
SELECT 
    ID,
    IF(COLONYRANK <= 0.25, 'CRITICAL',
      IF(COLONYRANK <= 0.5, 'HIGH',
        IF(COLONYRANK <= 0.75, 'MEDIUM', 'LOW'))) AS COLONY_NAME
FROM 
    ECOLI_DATA
INNER JOIN (
    SELECT 
        ID,
        PERCENT_RANK() OVER(ORDER BY SIZE_OF_COLONY DESC) AS COLONYRANK
    FROM 
        ECOLI_DATA
) AS RANKDATA
USING
